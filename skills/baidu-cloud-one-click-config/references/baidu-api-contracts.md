...
发布状态：`1` 未发布、`2` 发布中、`3` 成功、`4` 失败。只有 `3` 能进入号码流程。
...
创建任务时 `taskName` 不得重复。`isOpenEmptyNum`、`isOpenPhoneDown` 是必填布尔值，必须来自已确认草案。名单 `secretType=2` 会发送明文号码；除非用户明确选择且本地隐私规则允许，否则优先使用平台系统加密并要求有效 `secretId`。
...
证据允许保存：`requestId`、目标 `id/robotId/robotName`、任务 ID、脱敏号码、配置输入哈希、回读哈希、发布状态和外呼结果枚举。
