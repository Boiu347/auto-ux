...
当前网站通过一次配对的 Mac Agent 把结构化任务直接投递到 Codex `app-server daemon`。首次安装命令只依赖 macOS 自带工具，会自动安装并校验受管 Node.js 与 Codex 组件；日常使用不经过剪贴板、模拟按键或辅助功能权限。同一网站升级 Agent 与 Skill 时复用现有设备令牌，不要求重新配对。
